...
export const TransactionsProvider = ({ children }) => {
  ...
  //formdata1 for register gov  changes here
  const [formData1, setformData1] = useState({ address: "", govtype: "", name: "",govmail:"",govpin:""});
// changes here
  ...
  // changes here
  const [formData5,setformData5]=useState({dept_name:"",project_name:"",no_installments:"",amount:""});
...
const CreateProject = async () => {
  try {
  if (ethereum) {
    // considering formdata5 as concerned
      const { dept_name, project_name, no_installments,amount } = formData5;
      const transactionsContract = createEthereumContract();


      const transaction = await transactionsContract.Register(dept_name, project_name,no_installments,amount);
